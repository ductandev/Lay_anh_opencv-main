...
from cv2 import imread, resize, VideoCapture, cvtColor, imwrite, COLOR_BGR2RGB, CAP_PROP_FPS, putText, rectangle, FONT_HERSHEY_SIMPLEX, LINE_AA, CAP_DSHOW, CAP_PROP_FRAME_WIDTH, CAP_PROP_FRAME_HEIGHT
from os import  path,  makedirs
import sys
from PyQt5.QtWidgets import (QToolBar, QLineEdit, QAction, QFileDialog, QAbstractItemView, QMessageBox)
from PyQt5.QtCore import  Qt, QItemSelectionModel, QAbstractTableModel, QModelIndex, QVariant, QSize 
...
class Ui_add_image_2(object):
        ...
        if ret:
            if  (self.lenImage != -1):
                ...
                if(lenFaceDetect == 1):
                    ...
                    imwrite(r"Data\\{}\\{}.png".format(IDStudent, IDStudent),self.frame)
                    # imwrite(r"Data\\Dataset\\FaceData\\raw\\{}\\{}.png".format(IDStudent, IDStudent),self.frame)
                ...
                if(self.lenImage == 1):         ######### số lượng ảnh muốn lấy
                    self.lenImage = -1
                    self.millisecs = 10
                    # self.video_add_image_2.setPixmap(QtGui.QPixmap(r"Finish.png"))
    ...
    def showSelection(self, item):
        ...
        statusimg = None
        if path.exists(r"Data\\{}\\{}.png".format(str(sfd[1]), str(sfd[1]))):
            statusimg = "Yes"
            self.status_label_add_image_2.setText("Exist data student {}".format(str(sfd[1])))
            imwrite(r"Assets\\tam.png", resize(imread(r"Data\\{}\\{}.png".format(str(sfd[1]), str(sfd[1]))),(200, 150)))
        else:
            self.status_label_add_image_2.setText("Statu: None ")
            imwrite(r"Assets\\tam.png", resize(imread(r"Assets\\none4.png"),(200, 150)))
        noidung = "FullName : {}.\nTitkulcode: {}.\nClass: {}.\nStatus: {}.".format(sfd[3],sfd[1], sfd[4], statusimg)
        self.status_label_info.setText(noidung)
        pixmap = QPixmap("Assets\\tam.png")
    ...
    def writeCSV(self):
        fileName, _ = QFileDialog.getSaveFileName(None, "Open File", self.filename,"CSV Files (*.csv) ;; Excel Workbook (*.xlsx)")
        if fileName:
    ...
    def removepath(self, i):
        ...
        if str(i.text()) == "OK":
            try:
                rmtree("Data\\{}".format(str(self.IDStudentss)))
    ...
    def __init__(self):
        ...
        self.button_change_camera = QtWidgets.QPushButton(add_image_2)
        ...
        self.label_text_version = QtWidgets.QLabel(add_image_2)
    ...
    def setupUi(self, add_image_2):
        ...
        self.cap.set(CAP_PROP_FRAME_WIDTH, 1280)
        self.cap.set(CAP_PROP_FRAME_HEIGHT, 720)
        add_image_2.setWindowIcon(QtGui.QIcon(r"Assets\\icon.png"))
        add_image_2.setWindowTitle("Program take picture")
        add_image_2.setObjectName("add_image_2")
        add_image_2.resize(640, 600)
        add_image_2.setMinimumSize(QtCore.QSize(1380, 850))
        add_image_2.setMaximumSize(QtCore.QSize(1380, 850))
        ...
        self.lb.setGeometry(650, 200 , 710, 625)
        ...
        self.openAct = QAction(QIcon(r"Assets\\open4.png"), 'Open', triggered=self.loadCSV, shortcut = QKeySequence.Open)
        self.saveAct = QAction(QIcon(r"Assets\\save4.png"), 'Save', triggered=self.writeCSV_update, shortcut = QKeySequence.Save)
        self.saveAs = QAction(QIcon(r"Assets\\saveas4.png"), 'Save as', triggered=self.writeCSV, shortcut = QKeySequence.SaveAs)
        self.removeACt = QAction(QIcon(r"Assets\\remove4.png"), 'Remove', triggered=self.removeimage, shortcut = QKeySequence.Delete)
        self.change_cam = QAction(QIcon(r"Assets\\camera5.png"), 'Change Camera', triggered=self.change_camera, shortcut = QKeySequence.Open)
        ...
        self.Remove_Act = self.menu.addAction(self.removeACt)
        self.Change_cameraa_tb = self.menu.addAction(self.change_cam)

        self.lineFind = QLineEdit(add_image_2)
        self.lineFind.setGeometry(650, 10 , 710, 30)
        ...
        self.button_change_camera.setGeometry(QtCore.QRect(415, 776, 205, 41))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.button_change_camera.setFont(font)
        self.button_change_camera.setIcon(QIcon(r"Assets\\camera4.png"))
        self.button_change_camera.setIconSize(QtCore.QSize(50,50))
        self.button_change_camera.setObjectName("button_change_camera")
        """---------------------------------------------------------------"""
        self.button_change_camera.clicked.connect(self.change_camera)
        """---------------------------------------------------------------"""
        ...
        self.status_label_image.setGeometry(QtCore.QRect(650, 45, 200, 150))
        imwrite(r"Assets\\tam.png", resize(imread( r"Assets\\none4.png"),(200, 150)))
        pixmap = QPixmap("Assets\\tam.png")
        self.status_label_image.setPixmap(pixmap)
        
        self.label_text_version.setGeometry(QtCore.QRect(1250, 822, 210, 30))
        font = QtGui.QFont()
        font.setPointSize(12)
        self.label_text_version.setFont(font)
        self.label_text_version.setObjectName("text_version")
    ...
    def retranslateUi(self, add_image_2):
        ...
        self.button_change_camera.setText(_translate("add_image_2", "Change Camera"))
        ...
        self.label_text_version.setText(_translate("add_image_2", "Version_0.1"))
