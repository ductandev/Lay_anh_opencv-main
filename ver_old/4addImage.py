...
class Ui_add_image_2(object):   
    def getIDStudent(self):            
        ...
        if ( IDStudent.isdigit() == True ):
            ...
            try:
                if (int(IDStudent) == 0):   
                    self.cap.release()                    
                print(makedirs(".\\Data\\Dataset\\FaceData\\raw\\" + str(IDStudent)))
                ...
                self.lenImage = -1
                try:
                    if path.exists(".\\Data\\Dataset\\FaceData\\raw\\" + str(IDStudent)):
                        self.status_label_add_image_2.setText("Exist data student {}".format(IDStudent))
                    else:
                        makedirs(".\\Data\\Dataset\\FaceData\\raw\\" + str(IDStudent))
                        print("dont's")
        ...
        if ret:
            if  (self.lenImage != -1):
                ...
                if(lenFaceDetect == 1):
                    ...
                    imwrite(".\\Data\\Dataset\\FaceData\\raw\\" + str(IDStudent) + "\\Image{}.png".format(self.lenImage),self.frame)
                    self.frame = putText(self.frame, "MNV: {}".format(self.names), (20,60), FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, LINE_AA)
                if(self.lenImage == 100):
                    self.lenImage = -1
                    self.millisecs = 10
    ...
    def setupUi(self, add_image_2):
        ...
        self.video_add_image_2.setGeometry(QtCore.QRect(0, 0, 1280, 720))
